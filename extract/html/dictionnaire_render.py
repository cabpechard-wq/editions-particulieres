"""Rendu HTML du dictionnaire (index A–Z, filtre, liens manuel)."""
...
def parse_entries_from_html(raw: str, title_map: dict[str, str]) -> list[dict]:
    ...
    for L in letters:
        ...
        for e in by_letter.get(L, []):
            blocks.append(
                ...
                f'<div class="dict-def">{e["definition_html"]}</div>'
            )
            if e["extras_html"]:
